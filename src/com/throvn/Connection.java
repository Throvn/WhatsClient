...
public class Connection
{
    ...
    public String receive()
    {
        try
        {
            if (zEingang != null) return zEingang.readLine();//Warten auf eine Nachricht ...
        }
        catch (IOException e)
        {
    ...
    private boolean isConnected() {return zSocket.isConnected();}
    
    private boolean isClosed() {return zSocket.isClosed();}
    
    private String getRemoteIP() {return "" + zSocket.getInetAddress();}
    
    private int getRemotePort() { return zSocket.getPort(); }    
    ...
    public void close()
    {
        ...
        {            
            if (zSocket != null)
                if (! zSocket.isClosed())
                {
                    zAusgang.close();zEingang.close();zSocket.close();
                }
        }
        catch (IOException e)
        {
